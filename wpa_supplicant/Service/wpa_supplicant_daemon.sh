...
#---VARIABLES
connection_isfully_established=false
...
Connection_Check_And_Data_Retrieval() {
    ...
    local PHASE_IPV4_RETRIEVE="2"
    local PHASE_IPV6_RETRIEVE="3"
    local PHASE_EXIT="4"

    #Define variables
    local phase="${PHASE_SSID_CONNECTION_CHECK}"
    local ctr=0
    local getssid="${EMPTYSTRING}"
        ...
        case "${phase}" in
            "${PHASE_SSID_CONNECTION_CHECK}")
                ...
                do
                    ...
                    getssid=$(iwgetid -r); exitcode=$?
                    #In case exitcode != 0, then get the result from log-file
                    if [[ ${exitcode} -ne 0 ]]; then
                        getssid=$(sudo grep "${PATTERN_WLAN0_CLTR_EVENT_CONNECTED}" "${WPA_WLAN0_LOG_FPATH}")
                    fi

                    #Check if 'getssid' contains a value
                    if [[ -n "${getssid}" ]]; then
                        #Update boolean
                        ssid_isconnected=true
                ...
                #Print
                if [[ ${ssid_isconnected} == true ]]; then
                    echo -e ":-->${PRINT_STATUS}: ${PRINT_CONNECTED_TO} ${FG_LIGHTGREY}${getssid}${NOCOLOR}: ${PRINT_SUCCESSFUL}"

                    phase="${PHASE_IPV4_RETRIEVE}"
                else
                    echo -e ":-->${PRINT_STATUS}: ${PRINT_CONNECTED_TO} ${FG_LIGHTGREY}${getssid}${NOCOLOR}: ${PRINT_FAILED}"
            ...
            "${PHASE_IPV6_RETRIEVE}")
                ...
                ;;
            "${PHASE_EXIT}")
                #Determine 'connection_isfully_established'
                if [[ ${ssid_isconnected} == false ]]; then
                    connection_isfully_established=false
                else    #ssid_isconnected = true
                    if [[ ${ipv4isfound} == false ]] && [[ ${ipv6isfound} == false ]]; then
                        connection_isfully_established=false
                    else    #ipv4isfound = true and/or ipv6isfound = true
                        connection_isfully_established=true
                    fi
                fi
...
WriteToFile() {
    ...
    #Print
    if [[ ${exitcode} -eq 0 ]]; then
        echo -e ":-->${PRINT_STATUS}: ${PRINT_WRITE_TO} ${FG_LIGHTGREY}${istargetfpath}${NOCOLOR}: ${PRINT_DONE}"
    else
        echo -e ":-->${PRINT_STATUS}: ${PRINT_WRITE_TO} ${FG_LIGHTGREY}${istargetfpath}${NOCOLOR}: ${PRINT_FAILED}"
    fi
}
...
Wpa_Supplicant_Daemon_Service_DisableStop() {
    ...
    #Define variables
    local disable_cmd="systemctl disable ${srv_name}"
    local stop_cmd="systemctl stop ${srv_name}"
...
Start_Handler() {
    ...
    local PHASE_IP_FLUSH_ALL="3"
    local PHASE_WPA_WLAN0_LOG_CLEAR="4"
    local PHASE_INTFSTATE_CHECK_AND_SET="5"
    local PHASE_NETPLAN_APPLY="6"
    local PHASE_WPA_APPLICANT_PROCS_KILL_2NDTIME="7"
    local PHASE_WPA_APPLICANT_NETPLAN_DAEMON_START="8"
    local PHASE_CONNECTION_STATUS_CHECK="9"
    local PHASE_WRITE_TO_FILE="10"
    local PHASE_EXIT="11"
    ...
    do
        case "${phase}" in
            ...
            "${PHASE_NETPLAN_APPLY}")
                ...
                #   ...netplan apply MUST be run for the changes
                #   ...to take effect!!!
                #2. netplan apply is executed to (re)create the 
                #   ...file 'wpa-wlan0.conf'.
                CmdExec "${netplan_apply_cmd}"

                #Goto next-phase
                phase="${PHASE_WPA_APPLICANT_PROCS_KILL_2NDTIME}"
                ;;
            "${PHASE_WPA_APPLICANT_PROCS_KILL_2NDTIME}")
                #Kill ALL wpa_supplicant daemons
                #   These daemons include:
                #   1. wpa_supplicant.service daemon
                #   2. initiated via command: /sbin/wpa_supplicant
            ...
            "${PHASE_CONNECTION_STATUS_CHECK}")
                ...
                #If successful -> ret_failed = false
                #If failed -> ret_failed = true
                if [[ ${ret} == true ]]; then   #successful
                    #Check SSID connection status -> determine 'connection_isfully_established' {true|false}
                ...
                if [[ ${ret_failed} == false ]]; then
                    if [[ ${connection_isfully_established} == true ]]; then   #connected
                        #Update status
                        connection_status="${CONNECTED}"

                        #Goto next-phase
                        phase="${PHASE_WRITE_TO_FILE}"
                    else    #not connected
                    ...
                    #Update status
                    connection_status="${FAILED_TO_CONNECT}"
            ...
            "${PHASE_WRITE_TO_FILE}")
                ...
                ;;
            "${PHASE_EXIT}")
                if [[ ${connection_isfully_established} == false ]]; then   #not connected
                    #Remark:
                    #   Do NOT disable 'wpa_supplicant_netplan_daemon_kill.service',
                    #       because this daemon can help to kill the wpa_supplicant daemon, 
                    #       which is launched by netplan at boot.
                    Wpa_Supplicant_Daemon_Service_DisableStop "${WPA_SUPPLICANT_DAEMON_SRV}"
                fi
